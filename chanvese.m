...
%------------------------------------------------------------------------
function seg = chanvese(I,init_mask,max_its,alpha,display, max_area)
...
%-- default value for max area segmented
% if(~exist('max_area','var'))
%     max_area = 10000;
% end
...
%-- Initialize var prev_area to compare with the current area and break
prev_area = 0;
%--main loop
for its = 1:max_its   % Note: no automatic convergence test
    ...
    %-- Get current area (all white pixels of mask)
    curr_area = sum(seg(:) == 1);
    
    %-- Break if exceeds max_area
    if curr_area > max_area
        break
    end
    
    %-- Break if area decreases
